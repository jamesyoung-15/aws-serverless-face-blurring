...
const showErrorModal = () => {
  addModalMessage(
    "Error uploading image! Either your file has issues or the server ran into problem. Sorry!",
    "has-text-danger",
  );
  // show close button
  ...
  const result = await new Promise((resolve, reject) => {
    ...
    new Compressor(imageFile, {
      ...
      success(result) {
        ...
        addModalMessage(
          `Compressed from ${returnFileSize(fileSize)} to ${returnFileSize(result.size)}.`,
        );
...
confidenceButton.addEventListener("click", () => {
  ...
  let confidenceThreshold = document.getElementById("confidence-threshold").value;
  if (
    confidenceThreshold == "" ||
    ...
    document
      .getElementById("confidence-threshold-error")
  ...
  if (imageFile) {
    ...
    uploadImage(imageFile, confidenceThreshold)
      .then((uploadResponse) => {
